...
module.exports.run = async (bot, message, args) => {
    ...
    if (!message.guild.me.hasPermission("BAN_MEMBERS")) return message.channel.send('My role does not have `BAN_MEMBERS` permission.');
    let reason = args.splice(1).join(" ");
    let userID = args[0];
    if (!reason) reason = 'No reason given.';
    if (!args[0]) return message.channel.send('You must someone to unban.');
...
/***
 * Exports the unban command to the help object
 */
module.exports.help = {
    name: "unban",
    aliases: []
