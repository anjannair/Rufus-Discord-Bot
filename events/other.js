...
module.exports = async message => {
    let check;
    try {
        if (message.content.slice(1, 6).toLowerCase() == "clear" || message.content.slice(1, 5).toLowerCase() == "stop") {
            message.channel.send(`Don't forget to use \`${message.content.slice(0, 1)}leave\` when you're done listening so that the bot leaves the voice channel. Saves its resources!`);
        }
